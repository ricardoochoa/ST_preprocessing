...
cdmx  <- readOGR('_input/cdmx/cdmx.shp')

# If needed, transform the polygon to match CRS between layers. 
cdmx <- spTransform(x = cdmx, CRSobj = '+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs')

plot(built_up)
plot(cdmx, axes = TRUE, add = TRUE)
...
```{r mask raster}
masked <- mask(x = built_up, mask = cdmx)
plot(masked)
...
```{r write raster}
writeRaster(cropped, filename = "_output/cdmx_builtup.tif", overwrite = TRUE)
```
